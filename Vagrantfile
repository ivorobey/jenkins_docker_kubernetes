...
echo 'Environment="KUBELET_EXTRA_ARGS=--node-ip=192.168.0.#{i + 200}"' | sudo tee -a /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
...
for (( i=1; i <= #{NODE_COUNT}; i++ )); do
  addr=$((200 + $i))
  echo "192.168.0.$addr node$i" >> /etc/hosts
done
#echo "192.168.0.201 node1" >> /etc/hosts
# disable swap
swapoff -a
...
# Start cluster
sudo kubeadm init --apiserver-advertise-address=192.168.0.200 --pod-network-cidr=10.244.0.0/16 | grep -Ei "kubeadm join|discovery-token-ca-cert-hash" > ${OUTPUT_FILE}
chmod +x $OUTPUT_FILE
...
# Fix kubelet IP
echo 'Environment="KUBELET_EXTRA_ARGS=--node-ip=192.168.0.200"' | sudo tee -a /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
